import logging
from dash import callback, register_page, dcc, html, Input, State, Output, exceptions, callback_context, no_update, dash_table
...
def get_info(feature=None, space_agg=None):
    ...
    if space_agg == "TAZ":
        return header + [html.B(feature["properties"]["taz_name"] + " - " + str(feature["properties"]["taz_id"])), html.Br(),
                         "Approx: {:0.0f} terminals".format(feature["properties"]["sum_terminals"]), html.Br()]
...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.Label('Temporal resolution:'),
            dcc.Dropdown(
    ...
    html.Div([
        html.Div([html.Span("1", className="section-number"),
                  "Explore population density"], className="section-label"),
    ...
    html.Div([
        html.Div([html.Span("2", className="section-number"),
                  "Decompose and rank regions"], className="section-label"),
        dbc.Row(children=[
            ...
            dbc.Col(children=[
                    ...
                    html.Label("Time range to perform TS Decomposition:", style={
                        "fontWeight": "bold", "margin-top": "15px"}),
                    ...
                    html.Div([dbc.Progress(id="tsdecomp_progress_bar", value="0", style={"visibility": "hidden"})],  style={
    ...
    html.Div([
        html.Div([html.Span("3", className="section-number"),
                  "Map the actionable statistics"], className="section-label"),
        dbc.Row(dbc.Col([
            html.Label('Statistic to map:'),
            dcc.Dropdown(
                options={
                    'F_T': 'F_T',
                    'ROC':'ROC',
        ...
        ], width=4), justify="start", style={"margin-bottom": "15px"}),
...
def info_hover_statistics(feature, space_agg):
    return get_info_statistics(feature, space_feature(space_agg))
...
def update_checklist_from_map_and_spacedropdown(selected_space, space_agg, time_agg, checklist_values):
    ...
    if callback_context.triggered_id == "spaceagg-dropdown":
        gdf_data = get_current_dataset(time_agg, space_agg)
        space_names = sorted(gdf_data[space_feature(space_agg)].unique())
...
def update_figure_and_slider(time_agg, space_agg, data_feature, selected_time_range_agg):
    ...
    space_feature_name = space_feature(space_agg)
    wkt_feature_name = space_wkt(space_agg)

    gdf_data = get_current_dataset(time_agg, space_agg)

    sorted_dates = sorted(gdf_data['datetime'].unique())
    numdate = [x for x in range(len(sorted_dates))]

    if callback_context.triggered_id == "range-slider-mobility":
        selected_range = [sorted_dates[selected_time_range_agg[0]],
                          sorted_dates[selected_time_range_agg[1]]]
    else:
        selected_range = [sorted_dates[0], sorted_dates[0]]
...
def process_batch(batch_idx, batch_size, group_names, table_df, space_feature_name, data_features, time_agg, decomp_algo, decomp_period):

    logging.info("processing batch: "+str(batch_idx))
    ...
    for i, group_name in enumerate(batch_group_names):
        ...
        for data_feature in data_features:
            ...
            plot_jobs.append((start_idx+i, res, data_feature, y_pred, formula, group_name))
    ...
    return decomp_resids, decomp_stats_table, plot_jobs
...
def perform_tscomp_ifnotin_cache(set_progress, url, n_clicks, decomp_algo, decomp_periods, stored_decomp_resid,
    ...
    if checklist_values:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(process_batch, batch_idx, batch_size, group_names, table_df, space_feature_name, data_features, time_agg, decomp_algo, decomp_period) for batch_idx in range(num_batches)]
